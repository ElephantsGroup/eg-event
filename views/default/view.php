...
<header>
	<div class="header-content">
		<div class="header-content-inner">
			<a href="<?= Yii::getAlias('@web') ?>/event-list/index" style="color: white"><h1 id="homeHeading"><?= Yii::t('app', 'Events List')?></h1></a>
			<hr>
			<p><?= $model['title'] ?></p>
		</div>
	</div>
				...
				<div class="post">
					...
					<div class="post-content">
						...
						<div class="row">
							<div>
								<?php
								if($module->enabled_like) echo \elephantsGroup\like\components\Likes::widget(['item' => $model->id, 'service' => 3]);
								?>
							</div>
							<div class="col-md-6" style="float: right">
								<?php
								if($module->enabled_rating) echo \elephantsGroup\starRating\components\Rate::widget(['item' => $model->id, 'service' => 3]);
								?>
							</div>
						</div>
						...
						<?php if($module->enabled_comment):?>
							...
							<div>
								<?php
								echo \elephantsGroup\comment\components\LastComments::widget(['item' => $model->id, 'service' => 3]);
								?>
							</div>
							<?php
